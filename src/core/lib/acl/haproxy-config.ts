...
export function generateHaproxyConfig(options: HaproxyConfigOptions = {}): GeneratedHaproxyConfig {
  ...
  l.push(
    ...
    "",
    "global",
    // 16384 covers the longest request head haproxy itself accepts
    // (tune.bufsize - tune.maxrewrite); anything longer never reaches a rule.
    // The default 1024 cuts a line mid-URL, and a cut line stops parsing
    // entirely, taking the whole event out of the report. `len` has to precede
    // `format`: these arguments are positional, and the other order is a fatal
    // config error rather than a warning.
  ...
  if (ipRules.length > 0 || tlsHosts.length > 0) {
    ...
    l.push(
      ...
      // The SNI is last for the same reason as the inspected frontends' URL:
      // it is the one field whose length the build chooses, so whatever cuts a
      // line can only cost the name, never the decision before it.
  ...
  const stage = (
    ...
    l.push(
      ...
      // The URL comes last because it is the only field the build controls the
      // length of: every fixed-width field is then ahead of anything that could
      // cut the line, so a cut costs the tail of a URL and not a decision.
